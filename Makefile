...
host=$(shell grep host .config | sed 's/host //')
dbname=$(shell grep dbname .config | sed 's/dbname //')
username=$(shell grep username .config | sed 's/username //')
#autogenerated name
...
%.$(names).$(projects).from.$(startdate).to.$(enddate).csv: %.sql
	psql --host=$(host) --dbname=$(dbname) --username=$(username) --field-separator="," --no-align --tuples-only \
...
login:
	psql --host=$(host) --dbname=$(dbname) --username=$(username)
clean: 
