...
-- philippeg 30may2014
--
select to_char(j.created, 'YYYY'),to_char(j.created, 'WW'),concat(p.pkey,'-',j.issuenum),pri.pname,j.assignee,pv.vname,c.cname
from  jiraissue j 
...
LEFT OUTER JOIN priority pri ON
	pri.id=j.priority
LEFT OUTER JOIN nodeassociation nacomp ON 
	nacomp.ASSOCIATION_TYPE='IssueComponent'
	and nacomp.SOURCE_NODE_ID=j.id
LEFT OUTER JOIN component c ON 
	c.id=nacomp.SINK_NODE_ID
